...
uses unit_principal, udmConn, uBiblioteca, ufrmEstoquesMovimentacaoE,
  ufrmPesquisaProdutos, unit_funcoes, uTipos, unit_movimenta_estoques;
...
begin
  ...
  try
    ...
    begin
     ...
     {verifica se o estoque fisico na tabela de produtos esta diferente do saldo na tabela de moviemnetcao_estoque
     se for diferente atualiza o valor na tabela de produtos}
     qry.Last;
     unit_movimenta_estoques.prc_validar_estoque_fisico(qry.FieldByName('PRODUTO_ID').AsInteger,
                                                qry.FieldByName('ESTOQUE_FISICO').AsFloat);
...
begin
  ...
  try
    ...
    loqry.SQL.Add('  sum(i.qtde) as QTDE            ');
    loqry.SQL.Add('from                           ');
    loqry.SQL.Add('  pedidos p, pedidos_itens i                ');
    loqry.SQL.Add('where                          ');
    loqry.SQL.Add('  p.id = i.pedido_id and                          ');
    loqry.SQL.Add('  p.orcamento = :orcamento and                          ');
    loqry.SQL.Add('  i.PRODUTO_ID = :PRODUTO_ID and ');
    loqry.SQL.Add('  p.situacao = :situacao         ');
    ...
    loqry.SQL.Add('from                           ');
    loqry.SQL.Add('  pedidos p, pedidos_itens_laje i          ');
    loqry.SQL.Add('where                          ');
    loqry.SQL.Add('  p.id = i.pedido_id and                          ');
    loqry.SQL.Add('  p.orcamento = :orcamento and                          ');
    loqry.SQL.Add('  i.PRODUTO_ID = :PRODUTO_ID and ');
