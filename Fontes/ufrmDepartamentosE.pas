...
type
  TfrmDepartamentosE = class(TfrmBaseEdicao)
    Label1: TLabel;
    edID: TDBEdit;
    Label7: TLabel;
    eddataCad: TDBEdit;
...
procedure Incluir;
var
  loForm : TfrmDepartamentosE;
begin
  loForm := TfrmDepartamentosE.Create(Application);
  try
...
procedure TfrmDepartamentosE.btnOkClick(Sender: TObject);
begin
  inherited;
  Salvar();
...
procedure TfrmDepartamentosE.Componentes;
begin
  qry.Open('select * from '+ self.Tabela +' where ID = :ID');
end;

procedure TfrmDepartamentosE.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmDepartamentosE.FormShow(Sender: TObject);
begin
  inherited;
  Componentes;
  Inicializar;
...
procedure TfrmDepartamentosE.Inicializar;
begin
...
procedure TfrmDepartamentosE.LerDados;
begin
  uBiblioteca.FilterCds(qry, uTipos.fsInteger, inttostr(self.Codigo));

end;

procedure TfrmDepartamentosE.Salvar;
begin
  case Self.Operacao of
...
function TfrmDepartamentosE.valida: boolean;
var
  loQuery: TFDQuery;
begin
  result := false;
