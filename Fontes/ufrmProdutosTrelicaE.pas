...
type
  TfrmProdutosTrelicaE = class(TfrmBaseEdicao)
    ...
    qryTrelicaID: TIntegerField;
    qryTrelicaPRODUTO_ID: TIntegerField;
    qryTrelicaALTURA: TIntegerField;
...
begin
  ...
  uTipos.opIncluir: begin
                       pnTitulo.Caption := 'CADASTRO DE TRELIÇAS';
...
begin
  ...
  cbx_tipo_trelica.ItemIndex := qryTrelica.FieldByName('TIPO_TRELICA').AsInteger ;
end;
...
begin
  ...
  begin
    ShowMessage('Informe a ALTURA da treliça');
    cbx_altura.SetFocus;
    exit;
  end;
  ...
  begin
    ShowMessage('Informe a medida do ferro SUPERIOR da treliça');
  ...
  begin
    ShowMessage('Informe a medida do ferro SINUZÓIDE da treliça');
  ...
  begin
    ShowMessage('Informe a medida do ferro INFERIOR da treliça');
  ...
  begin
    ShowMessage('Informe o COMPRIMENTO DA TRELIÇA');
  ...
  begin
    ShowMessage('Informe o TIPO DE TRELIÇA');
    cbx_tipo_trelica.SetFocus;
    exit;
  end
  else
  begin
    qryTrelica.FieldByName('TIPO_TRELICA').AsInteger := cbx_tipo_trelica.ItemIndex;
  end;
  ...
  begin
    ShowMessage('Informe o PREÇO DE FORNECEDOR');
    edCusto.SetFocus;
    exit;
  end;
...
begin
  ...
  qryTrelica.FieldByName('PRECO_CUSTO').AsFloat := custo_liquido;
...
begin
  ...
  p_descricao_fantasia := 'TRELIÇA H-' + floattostr(altura) + ' BARRA COM ' + formatfloat('0.00', strtofloat(edt_comprimento.Text)) + ' MT(S) "' + cbx_tipo_trelica.text + '"';
 // edt_descricao_tecnica.Text := p_descricao_tecnica;
 // edt_descricao_fantasia.Text := 'TRELIÇA H-' + floattostr(altura) + ' BARRA COM ' + edt_comprimento.Text + ' MT(S) "' + cbx_tipo_trelica.text + '"';
...
begin
  ...
  begin
    ShowMessage('Informe as dimensões corretas da treliça a ser cadastrada');
    exit;
  end
  else
    prc_descricao_trelica;
