...
type
  ...
  public
    ...
    property p_alterado: boolean read Fp_alterado write Fp_alterado;


  end;

var
  frmComissaoFechamento_Alt_Valor: TfrmComissaoFechamento_Alt_Valor;
(*
...
procedure TfrmComissaoFechamento_Alt_Valor.btnOkClick(Sender: TObject);
begin
  inherited;
  if fnc_validar then
  close;
...
begin
  ...
  if ( (StrToFloatDef(edt_pagar.Text,0) <= 0) or
       (StrToFloatDef(edt_pagar.Text,0) > (p_valor_comissao+0.0001) ) )
  then
  begin
  ...
  if StrToFloatDef(edt_pagar.Text,0) < (p_valor_comissao-0.0001) then
  begin
    p_alterado := true;
    //ShowMessage('edt < p_valor');
  end;

  if p_alterado = false then
    if edt_historico.Text <> p_historico then
    begin
      p_alterado := true;
      //ShowMessage('edt <> p_historico');
    end;
...
begin
  ...
  edt_pagar.Text       := formatfloat('0.00', p_valor_pago);
