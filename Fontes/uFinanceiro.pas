...
interface
 uses
  ...
  procedure prc_incluir_movimento_caixa(tabela: string; tabela_id: integer; entrada_saida: string; historico: string; valor: double);
...
begin
  try
    ...
    qry.SQL.Add('select * from CONTAS_RECEBER where TABELA_ORIGEM = :TABELA_ORIGEM and ID_ORIGEM =:ID');
    qry.ParamByName('TABELA_ORIGEM').AsString := 'PEDIDOS';
    qry.ParamByName('ID').AsInteger           := pedidoId;
    qry.Open;
...
{procedure para manutencao da tabela de dinheiroMovimentacao}
procedure prc_incluir_movimento_caixa(tabela: string; tabela_id: integer;
                       entrada_saida: string; historico: string; valor: double);
var
 loQry : TFDQuery;
 saldo_atual : double;
begin
  ...
  try
    ...
    loQry.Close;
    loqry.SQL.Clear;
    loqry.SQL.Add('select id, saldo from DINHEIRO_MOVIMENTACAO order by id');
    loQry.Open;
    loQry.Last;

    // pega o último saldo
    saldo_atual := loQry.fieldbyname('SALDO').AsFloat;
    loQry.close;

    // incluir movimento
    loqry.SQL.Clear;
    loQry.SQL.Add('insert into DINHEIRO_MOVIMENTACAO ' );
    loQry.SQL.Add('(');
    ...
    begin
      loQry.ParamByName('ENTRADA').AsFloat := Valor;
      loQry.ParamByName('SAIDA').AsFloat   := 0;
      loQry.ParamByName('SALDO').AsFloat   := saldo_atual + Valor
    end
    else
    if entrada_saida = 'SAIDA' then
    begin
      loQry.ParamByName('ENTRADA').AsFloat := 0;
      loQry.ParamByName('SAIDA').AsFloat   := Valor;
      loQry.ParamByName('SALDO').AsFloat   := saldo_atual - Valor;
    end;

    loQry.ParamByName('TABELA_ORIGEM').AsString     := tabela;
    loQry.ParamByName('TABELA_ORIGEM_ID').AsInteger := tabela_id;
    loQry.ExecSQL;
...
function baixar_contas_receber(conexao: TFDConnection;
                               contas_receber_id: integer;
                               valor_a_alterar, valor_pago: double): boolean;
...
begin
  try
    ...
    qry.Connection := conexao;
    qry.Sql.clear;
    qry.Sql.Add('SELECT id, VALOR_PAGO, SALDO_ABERTO FROM CONTAS_RECEBER WHERE ID = :ID');
    qry.ParamByName('ID').AsInteger := contas_receber_id;
    qry.Open;
    ...
    qry.SQL.Clear;
    qry.Sql.Add('update                         ');
    qry.Sql.Add('  CONTAS_RECEBER               ');
    qry.Sql.Add('set                            ');
    qry.Sql.Add('  DATA_PAGTO = :DATA_PAGTO,    ');
    ...
    qry.ParamByName('DATA_PAGTO').AsDate   := Date;
    qry.ParamByName('VALOR_PAGO').AsFloat  := oldValorPago + valor_pago;
    qry.ParamByName('SALDO_ABERTO').AsFloat:= oldSaldoAberto - valor_pago;
    qry.ParamByName('PAGO').AsString       := uBiblioteca.SeSenao(( oldSaldoAberto - valor_pago) < 0.01,'S','N') ;
    qry.ParamByName('ID').AsInteger        := contas_receber_id;
    qry.ExecSQL;
    //
    result := qry.RowsAffected > 0;
...
begin
  try
    ...
    qry.SQL.Clear;
    qry.SQL.Add('select id, VALOR_TOTAL_PEDIDO, VALOR_PAGO from PEDIDOS where ID =:ID');
