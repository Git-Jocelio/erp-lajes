...
type
  TfrmComissaoFechamentoE = class(TfrmBaseEdicao)
    ...
    procedure dbg_comissoesTitleClick(Column: TColumn);
  private
    Fp_codigo: integer;
...
begin
  inherited;
  if fnc_validar then
    if Application.MessageBox('Confirma o FECHAMENTO da comissão', 'Fechamento de comissão', MB_YESNO) = mrYes then
      prc_salvar;
...
begin
  ...
  begin
    ...
    if Application.MessageBox('Confirma a EXCLUSÃO da comissão', 'Exclusão de comissão', MB_YESNO)= mrYes then
...
procedure TfrmComissaoFechamentoE.dbg_comissoesTitleClick(Column: TColumn);
begin
  inherited;
  mtb_comissoes.IndexFieldNames := Column.FieldName;
end;
