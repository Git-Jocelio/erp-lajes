...
begin
  ...
  qry_operadora.SQL.Add('select p.id, p.nome from  pessoas p, cartao_operadoras o where o.pessoa_id = p.id order by p.nome');
  qry_operadora.Open;
end;
...
begin
  ...
  if rg_selecao.ItemIndex = 0 then
  begin
    if StrToFloat(edt_vr_aluguel.Text) < 0 then
    begin
      ShowMessage('Informe um VALOR VÁLIDO');
      edt_vr_aluguel.SetFocus;
      exit;
    end;
...
begin
  ...
  qry.ParamByName('VALOR_ALUGUEL').AsFloat          := strtofloatdef( edt_vr_aluguel.Text,0 );
  qry.ParamByName('DIA_VENCIMENTO').AsInteger       := StrToIntDef(edt_dia_vencimento.Text,0);
