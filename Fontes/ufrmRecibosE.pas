...
//ShowMessage('ENTREI');
  ...
  //ShowMessage('incluir');
    ...
    loQry.ParamByName('VALOR').AsFloat        := mtb_dinheiro.FieldByName('VALOR').AsFloat;
    loQry.ParamByName('OBSERVACAO').asstring  := mtb_dinheiro.FieldByName('OBSERVACAO').AsString;
...
begin
  try
    ...
    begin
      ...
      begin
        mtb_dinheiro.Insert;
        mtb_dinheiro.FieldByName('ID').AsInteger := loqry.FieldByName('ID').AsInteger;
        mtb_dinheiro.FieldByName('recibo_id').AsInteger := loqry.FieldByName('recibo_id').AsInteger;
        mtb_dinheiro.FieldByName('valor').AsFloat := loqry.FieldByName('valor').AsFloat;
        mtb_dinheiro.FieldByName('observacao').AsString := loqry.FieldByName('observacao').AsString;
        mtb_dinheiro.Post;
        loqry.Next;
      end;
    end;
...
//ShowMessage('ENTREI');
  ...
  //ShowMessage('incluir');
    ...
    loQry.ParamByName('HISTORICO').AsString          := copy(historico,1,60);
    loQry.ParamByName('VALOR_PARCELA').AsFloat       := valor_parcela;
    loQry.ParamByName('VALOR_PAGO').AsFloat          := valor_pago;
    loQry.ParamByName('BAIXAR').AsFloat              := baixar;
...
begin
   ...
   total_em_dinheiro     := uBiblioteca.SeSenao( mtb_dinheiro.IsEmpty, 0, mtb_dinheiro.FieldByName('TOTAL').value);    total_em_cheques      := uBiblioteca.SeSenao( mtb_cheque_cliente.IsEmpty, 0, mtb_cheque_cliente.FieldByName('TOTAL').value);
   ...
   pnl_dinheiro.Caption := FormatFloat('0.00', total_em_dinheiro);
...
begin
  ...
  dbg_dinheiro.Columns[0].Visible := false;
  dbg_dinheiro.Columns[1].Visible := false;
...
begin
  ...
  //ShowMessage('incluir');
    ...
    loqry.ParamByName('VALOR').AsFloat := mtb_dinheiro.FieldByName('VALOR').AsFloat;
    loqry.ParamByName('OBS').AsString  := mtb_dinheiro.FieldByName('OBSERVACAO').AsString;
    {-1 será o valor default para entradas avulsas, pois não será vinculada a
    ...
    ufinanceiro.prc_incluir_movimento_caixa('RECIBO', Codigo, 'ENTRADA', mtb_dinheiro.FieldByName('OBSERVACAO').AsString, mtb_dinheiro.FieldByName('VALOR').AsFloat);
  finally
    freeandnil( loqry );
  end;
end;
...
begin
  try
    ...
    loqry.parambyname('obs').AsString := copy( edt_obs_pix.Text, 1, 100);
    loqry.ExecSQL
  finally
    freeandnil( loqry );
  end;
