...
uses uBiblioteca, ufrmPesquisaProdutos, unit_funcoes, unit_movimenta_estoques;
...
begin
  ...
  try
    ...
    unit_movimenta_estoques.prc_atualizar_estoque( produto, tipo_movimento, 'AVULSO', strtofloat(edt_quantidade.Text) );

    {lança movimento de entrada na tabela de movimentação de estoques}
    unit_movimenta_estoques.prc_incluir_alterar_movimento( OpIncluir,tipo_movimento,'ESTOQUES_MOVIMENTACAO', -1, produto,edt_historico.Text, strtofloat(edt_quantidade.Text) );
    ...
    on E : Exception do
       begin
         CriarMensagem('AVISO','NÃO FOI POSSIVEL SALVAR O MOVIMENTO.' + slinebreak + slinebreak + E.Message);
