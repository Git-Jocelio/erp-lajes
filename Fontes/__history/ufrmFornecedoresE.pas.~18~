...
begin
  ...
  begin
    CriarMensagem('AVISO','Informe o nome do vendedor(a) desse fornecedor.');
  ...
  begin
    CriarMensagem('AVISO','Informe a descrição dos produtos que o fornecedores vende');
...
begin
  ...
  try
    loQry:= TFDQuery.Create(self);
    loQry.Connection := conexao;
    loqry.sql.Add('select P.*, F.* from PESSOAS P, FORNECEDORES F where P.ID = F.PESSOA_ID and P.ID = :ID');
...
begin
  ...
  try
    ...
    if Self.Conexao.InTransaction then Self.Conexao.Commit;
      CriarMensagem('AVISO','Fornecedor salvo com sucesso!');
