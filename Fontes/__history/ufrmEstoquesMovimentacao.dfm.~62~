inherited frmEstoquesMovimentacao: TfrmEstoquesMovimentacao
  Caption = 'frmEstoquesMovimentacao'
  ClientWidth = 1100
  ShowHint = True
  ExplicitWidth = 1100
  TextHeight = 15
  inherited GroupBoxPesquisa: TGroupBox
    Width = 1100
    Caption = ''
    ExplicitWidth = 1100
    inherited lbl_texto: TLabel
      Top = 6
      Width = 128
      Caption = 'Selecione um Produto'
      ExplicitTop = 6
      ExplicitWidth = 128
    end
    object btn_produto: TSpeedButton [1]
      Left = 4
      Top = 42
      Width = 50
      Height = 44
      Cursor = crHandPoint
      Hint = 'Buscar um produto'
      Glyph.Data = {
        
        6666666666666666666666666666666666666666666666666666}
      OnClick = btn_produtoClick
    end
    object lbl_descricao_produto: TLabel [2]
      Left = 62
      Top = 43
      Width = 270
      Height = 37
      Caption = 'Selecione um produto'
      Font.Charset = ANSI_CHARSET
      Font.Color = clNavy
      Font.Height = -27
      Font.Name = 'Segoe UI Semibold'
      Font.Style = [fsBold]
      ParentFont = False
    end
    inherited btnLocalizar: TBitBtn
      Left = 649
      Top = 51
      Height = 29
      Action = actIncluir
      Caption = '&Incluir'
      Visible = False
      ExplicitLeft = 649
      ExplicitTop = 51
      ExplicitHeight = 29
    end
    inherited edt_consulta: TEdit
      Left = 840
      Top = 6
      Width = 97
      Enabled = False
      Visible = False
      ExplicitLeft = 840
      ExplicitTop = 6
      ExplicitWidth = 97
    end
    object GroupBox1: TGroupBox
      Left = 825
      Top = 19
      Width = 273
      Height = 79
      Align = alRight
      Caption = 'Listar a partir do dia : '
      TabOrder = 2
      DesignSize = (
        273
        79)
      object dtp_data_ini: TDateTimePicker
        Left = 24
        Top = 32
        Width = 193
        Height = 29
        Anchors = [akTop]
        Date = 45575.000000000000000000
        Time = 0.801080081015243200
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'Segoe UI Semibold'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        OnCloseUp = dtp_data_iniCloseUp
      end
    end
  end
  inherited ToolBar1: TToolBar
    Width = 1100
    ExplicitWidth = 1100
    inherited btnAlterar: TToolButton
      Visible = False
    end
    inherited btnExcluir: TToolButton
      Visible = False
    end
  end
  inherited StatusBar: TStatusBar
    Top = 434
    Width = 1100
    Height = 1
    ExplicitTop = 434
    ExplicitWidth = 1100
    ExplicitHeight = 1
  end
  inherited PageControl1: TPageControl
    Width = 1100
    Height = 229
    ExplicitWidth = 1100
    ExplicitHeight = 229
    inherited TabSheet1: TTabSheet
      ExplicitWidth = 1092
      ExplicitHeight = 219
      inherited Panel5: TPanel
        Width = 1092
        Height = 178
        ExplicitWidth = 1092
        ExplicitHeight = 178
        inherited DBGrid1: TDBGrid
          Width = 1052
          Height = 127
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'DATA'
              Title.Alignment = taCenter
              Title.Caption = 'Data'
              Width = 100
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'NOME_FANTASIA'
              Title.Alignment = taCenter
              Title.Caption = 'Descri'#231#227'o'
              Width = 200
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'HISTORICO'
              Title.Alignment = taCenter
              Title.Caption = 'Hist'#243'rico'
              Width = 300
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'ENTRADA'
              Title.Alignment = taCenter
              Title.Caption = 'Entrada'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'SAIDA'
              Title.Alignment = taCenter
              Title.Caption = 'Saida'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'ESTOQUE_FISICO'
              Title.Alignment = taCenter
              Title.Caption = 'Est. Fisico'
              Width = 123
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'UNIDADE'
              Title.Alignment = taCenter
              Title.Caption = 'Unidade'
              Width = 55
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'TABELA_ORIGEM'
              Visible = False
            end
            item
              Expanded = False
              FieldName = 'TABELA_ORIGEM_ID'
              Visible = False
            end>
        end
        inherited pnl_resultado: TPanel
          Width = 1052
          ExplicitWidth = 1052
          inherited lbl_resultado: TLabel
            Width = 261
            Height = 41
            Caption = 'Movimenta'#231#227'o do produto'
            ExplicitWidth = 261
          end
        end
      end
      object pnl_estoques: TPanel
        Left = 0
        Top = 178
        Width = 1092
        Height = 41
        Align = alBottom
        BevelOuter = bvNone
        ParentBackground = False
        TabOrder = 1
        object Label1: TLabel
          Left = 666
          Top = 0
          Width = 253
          Height = 41
          Align = alLeft
          AutoSize = False
          Caption = 'ESTOQUE DISPONIVEL'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
          ExplicitLeft = 577
        end
        object lbl_estoque_disponivel: TLabel
          Left = 919
          Top = 0
          Width = 155
          Height = 41
          Align = alLeft
          AutoSize = False
          Caption = '0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
          ExplicitLeft = 878
        end
        object Label3: TLabel
          Left = 0
          Top = 0
          Width = 209
          Height = 41
          Align = alLeft
          AutoSize = False
          Caption = '     ESTOQUE FISICO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
        end
        object lbl_estoque_fisico: TLabel
          Left = 209
          Top = 0
          Width = 88
          Height = 41
          Align = alLeft
          AutoSize = False
          Caption = '0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
        end
        object Label5: TLabel
          Left = 338
          Top = 0
          Width = 184
          Height = 41
          Align = alLeft
          Alignment = taCenter
          AutoSize = False
          Caption = 'PEDIDO ABERTO'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI Semibold'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
          ExplicitLeft = 313
        end
        object lbl_pedido_aberto: TLabel
          Left = 522
          Top = 0
          Width = 144
          Height = 41
          Align = alLeft
          AutoSize = False
          Caption = '0.00'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -21
          Font.Name = 'Segoe UI'
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
          ExplicitLeft = 497
        end
        object btn_listar_pedidos_abertos: TSpeedButton
          Left = 297
          Top = 0
          Width = 41
          Height = 41
          Hint = 'Listar Pedidos Abertos'
          Align = alLeft
          Caption = '...'
          Flat = True
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'Segoe UI'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
  end
  inherited pnl_cabecalho: TPanel
    Width = 1100
    ExplicitWidth = 1100
    inherited btn_fechar: TSpeedButton
      Left = 1030
      ExplicitLeft = 1030
    end
    inherited lbl_titulo: TLabel
      Width = 490
      Height = 56
      Caption = 'MANUTEN'#199#195'O / MOVIMENTA'#199#195'O DOS ESTOQUES'
      ExplicitWidth = 490
    end
    inherited pnl_separa_topo: TPanel
      Width = 1100
      ExplicitWidth = 1100
    end
  end
  inherited qry: TFDQuery
    Connection = dmConn.FDConnection
    SQL.Strings = (
      ' select'
      '   E.ESTOQUES_ID,'
      '   TABELA_ORIGEM,'
      '   TABELA_ORIGEM_ID, '
      '   E.PRODUTO_ID, '
      '   P.NOME_FANTASIA, '
      '   P.UNIDADE, '
      '   E.DATA, '
      '   E.HISTORICO, '
      '   E.ENTRADA, '
      '   E.SAIDA, '
      '   E.ESTOQUE_FISICO'
      ' from'
      '   ESTOQUES_MOVIMENTACAO E, PRODUTOS P'
      ' where'
      '  E.PRODUTO_ID = P.ID and               '
      '  P.ID = :PRODUTO_ID                    '
      'order by       '
      ' E.ESTOQUES_ID '
      '')
    Left = 632
    Top = 40
    ParamData = <
      item
        Name = 'PRODUTO_ID'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object qryESTOQUES_ID: TIntegerField
      FieldName = 'ESTOQUES_ID'
      Origin = 'ESTOQUES_ID'
      Required = True
    end
    object qryPRODUTO_ID: TIntegerField
      FieldName = 'PRODUTO_ID'
      Origin = 'PRODUTO_ID'
    end
    object qryNOME_FANTASIA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'NOME_FANTASIA'
      Origin = 'NOME_FANTASIA'
      ProviderFlags = []
      ReadOnly = True
      Size = 100
    end
    object qryDATA: TDateField
      FieldName = 'DATA'
      Origin = '"DATA"'
    end
    object qryHISTORICO: TStringField
      FieldName = 'HISTORICO'
      Origin = 'HISTORICO'
      Size = 100
    end
    object qryENTRADA: TFMTBCDField
      FieldName = 'ENTRADA'
      Origin = 'ENTRADA'
      DisplayFormat = '0'
      Precision = 18
      Size = 2
    end
    object qrySAIDA: TFMTBCDField
      FieldName = 'SAIDA'
      Origin = 'SAIDA'
      DisplayFormat = '0'
      Precision = 18
      Size = 2
    end
    object qryESTOQUE_FISICO: TFMTBCDField
      FieldName = 'ESTOQUE_FISICO'
      Origin = 'ESTOQUE_FISICO'
      EditFormat = '0'
      Precision = 18
      Size = 2
    end
    object qryUNIDADE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'UNIDADE'
      Origin = 'UNIDADE'
      ProviderFlags = []
      ReadOnly = True
      Size = 2
    end
    object qryTABELA_ORIGEM: TStringField
      FieldName = 'TABELA_ORIGEM'
      Origin = 'TABELA_ORIGEM'
      Size = 100
    end
    object qryTABELA_ORIGEM_ID: TIntegerField
      FieldName = 'TABELA_ORIGEM_ID'
      Origin = 'TABELA_ORIGEM_ID'
    end
  end
  object ds_produtos: TDataSource
    AutoEdit = False
    DataSet = qry_produtos
    Left = 568
    Top = 129
  end
  object qry_produtos: TFDQuery
    Connection = dmConn.FDConnection
    SQL.Strings = (
      'select '
      '  P.ID,'
      '  P.NOME_FANTASIA'
      'from'
      ' PRODUTOS P'
      'where '
      '  REVENDA = '#39'S'#39' or VIGA = '#39'S'#39
      'order by '
      '  NOME_FANTASIA')
    Left = 568
    Top = 160
  end
  object mtb_movimento: TFDMemTable
    Active = True
    Filtered = True
    FieldDefs = <
      item
        Name = 'ESTOQUES_ID'
        DataType = ftInteger
      end
      item
        Name = 'DATA'
        DataType = ftDate
      end
      item
        Name = 'NOME_FANTASIA'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'HISTORICO'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'ENTRADA'
        DataType = ftFloat
      end
      item
        Name = 'SAIDA'
        DataType = ftFloat
      end
      item
        Name = 'ESTOQUE_FISICO'
        DataType = ftFloat
      end
      item
        Name = 'UNIDADE'
        DataType = ftString
        Size = 2
      end
      item
        Name = 'TABELA_ORIGEM'
        DataType = ftString
        Size = 100
      end
      item
        Name = 'TABELA_ORIGEM_ID'
        DataType = ftInteger
      end>
    IndexDefs = <>
    IndexFieldNames = 'ESTOQUES_ID'
    FetchOptions.AssignedValues = [evMode]
    FetchOptions.Mode = fmAll
    ResourceOptions.AssignedValues = [rvSilentMode]
    ResourceOptions.SilentMode = True
    UpdateOptions.AssignedValues = [uvCheckRequired, uvAutoCommitUpdates]
    UpdateOptions.CheckRequired = False
    UpdateOptions.AutoCommitUpdates = True
    StoreDefs = True
    Left = 716
    Top = 43
    object mtb_movimentoESTOQUES_ID: TIntegerField
      FieldName = 'ESTOQUES_ID'
    end
    object mtb_movimentoDATA: TDateField
      FieldName = 'DATA'
    end
    object mtb_movimentoNOME_FANTASIA: TStringField
      FieldName = 'NOME_FANTASIA'
      Size = 100
    end
    object mtb_movimentoHISTORICO: TStringField
      FieldName = 'HISTORICO'
      Size = 100
    end
    object mtb_movimentoENTRADA: TFloatField
      FieldName = 'ENTRADA'
    end
    object mtb_movimentoSAIDA: TFloatField
      FieldName = 'SAIDA'
    end
    object mtb_movimentoESTOQUE_FISICO: TFloatField
      FieldName = 'ESTOQUE_FISICO'
    end
    object mtb_movimentoUNIDADE: TStringField
      FieldName = 'UNIDADE'
      Size = 2
    end
    object mtb_movimentoTABELA_ORIGEM: TStringField
      FieldName = 'TABELA_ORIGEM'
      Size = 100
    end
    object mtb_movimentoTABELA_ORIGEM_ID: TIntegerField
      FieldName = 'TABELA_ORIGEM_ID'
    end
  end
end
