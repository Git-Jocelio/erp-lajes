...
begin
  ...
  // dados da comissão
  edt_comissao_vendedor.Text := formatfloat('0.00', qryVendedor.FieldByName('COMISSAO_VENDEDOR').AsFloat);
  edt_comissao_adm.Text      := formatfloat('0.00', qryVendedor.FieldByName('COMISSAO_ADM').AsFloat);
...
begin
  ...
  begin
    try
      try
        ...
        begin
          criarMensagem('AVISO','Vendedor já está cadastrado no sistema');
          exit;
        end;
      finally
        FreeAndNil(loQuery);
  ...
  // dados da comissão
  qryVendedor.FieldByName('COMISSAO_VENDEDOR').AsFloat := strtofloat(edt_comissao_vendedor.Text);
  qryVendedor.FieldByName('COMISSAO_ADM').AsFloat := strtofloat(edt_comissao_adm.Text);


  //***
  criarMensagem('AVISO','Vendedor validado com sucesso!');
