...
type
  TfrmChequesClientes = class(TfrmBaseGrade)
    ...
    qryDATA_REPASSE: TDateField;
    qryREPASSADO_PARA: TStringField;
    qryDEVOLVIDO: TStringField;
    qryDATA_DEVOLUCAO: TDateField;
    qryDEVEDOR_PESSOA_ID: TIntegerField;
    ...
    procedure edt_valor_iniKeyPress(Sender: TObject; var Key: Char);
    procedure edt_valor_fimKeyPress(Sender: TObject; var Key: Char);
    procedure edt_valor_iniExit(Sender: TObject);
    procedure edt_valor_fimExit(Sender: TObject);
  private
    procedure prc_sql;
...
begin
  try
    ...
    frmChequesClientesE.dtp_repassado.Date   :=  qry.FieldByName('DATA_REPASSE').AsDateTime;
    frmChequesClientesE.edt_repassado.Text   :=  qry.FieldByName('REPASSADO_PARA').AsString;



    frmChequesClientesE.cb_devolvido.Checked :=  uBiblioteca.SeSenao(qry.FieldByName('DEVOLVIDO').AsString = 'S', TRUE, FALSE);
    frmChequesClientesE.dtp_devolvido.Date   :=  qry.FieldByName('DATA_DEVOLUCAO').AsDateTime;
    frmChequesClientesE.cbx_receber.KeyValue :=  qry.FieldByName('DEVEDOR_PESSOA_ID').AsInteger;
...
procedure TfrmChequesClientes.DBGrid1TitleClick(Column: TColumn);
begin
  inherited;
  btnLocalizar.Click;
end;

procedure TfrmChequesClientes.edt_valor_fimExit(Sender: TObject);
begin
  inherited;
  ubiblioteca.prc_formata_dinheiro(sender);
end;

procedure TfrmChequesClientes.edt_valor_fimKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
...
procedure TfrmChequesClientes.edt_valor_iniExit(Sender: TObject);
begin
  inherited;
  ubiblioteca.prc_formata_dinheiro(sender);
end;

procedure TfrmChequesClientes.edt_valor_iniKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  ubiblioteca.prc_somente_numeros(sender, key);
...
begin
  ...
  if rg_situacao_cheque.ItemIndex = 3 then
    condicao := ' and cc.repassado <> :repassado ';
  if rg_situacao_cheque.ItemIndex = 2 then
    condicao := ' and cc.devolvido = :devolvido ';


  if cb_data_cheque.Checked then
  begin
    condicao := condicao + ' and cc.pre_datado_para >= :data_ini and cc.pre_datado_para <= :data_fim ';
  end;
  ...
  if rg_situacao_cheque.ItemIndex = 2 then
    qry.ParamByName('devolvido').AsString := 'S';
  ...
  begin
    total := total + qry.FieldByName('valor').AsFloat;
    qry.Next;
  end;
  StatusBar.Panels[0].Text :=  'QTDE DE CHEQUE(S) "' + inttostr(qry.RecordCount) + '" VALOR TOTAL R$ ' + formatfloat('0.00', total);
end;
...
begin
  ...
  begin
    situacao_cheque := 'N'; // em carteira
  end else
  if rg_situacao_cheque.ItemIndex = 1 then
  begin
    situacao_cheque := 'S';  // repassados
  end else
  if rg_situacao_cheque.ItemIndex = 2 then
  begin
    situacao_cheque := 'D';     //devolvidos
  end else
  if rg_situacao_cheque.ItemIndex = 3 then
  begin
    situacao_cheque := '';     //devolvidos
  end;
