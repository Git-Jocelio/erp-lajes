...
var
  ...
  //dados da pessoa e conta caso o lançamento for pro contas a pagar
  pessoa_id, conta_id, opcao : integer;
  pessoa_nome, pessoa_endereco, historico : string;
  ...
  begin
    try
      ...
      begin
        ...
        pessoa_id       := frmSelecionaPessoa_Custo.pessoa_id;
        conta_id        := frmSelecionaPessoa_Custo.custo_id;
        pessoa_nome     := frmSelecionaPessoa_Custo.pessoa_nome;
        pessoa_endereco := frmSelecionaPessoa_Custo.pessoa_endereco;
        // gerar historico personalizado
        opcao           := frmSelecionaPessoa_Custo.opcao;
        if opcao = 0 then
        begin
          historico := '[ENTR DIN] ' + pessoa_nome + ' ' + pessoa_endereco + ' ' + frmSelecionaPessoa_Custo.pessoa_historico;
        end else
        if opcao = 1 then
        begin
          historico := '[ENTR DIN] PED N. ' + frmSelecionaPessoa_Custo.pessoa_historico;
        end else
        if opcao = 2 then
        begin
          historico := '[ENTR DIN] ' + frmSelecionaPessoa_Custo.pessoa_historico;
        end;
...
//ShowMessage('incluir');
  ...
  if ( cb_contas_pagar.Checked ) and ( pessoa_id > 0 )then
  begin
