...
begin
  try
    ...
    begin
      ...
      loQry.SQL.Add('  DESCRICAO_SERVICO,     ');
      loQry.SQL.Add('  ATIVO                  ');
      loQry.SQL.Add(')                        ');
    ...
    begin
      ...
      loQry.SQL.Add('  DESCRICAO_SERVICO  = :DESCRICAO_SERVICO, ');
    ...
    loQry.ParamByName('DESCRICAO_SERVICO').AsString := edt_servicos.Text;
...
begin
  lbl_titulo.Caption := 'PRESTADORES DE SERVIÇOS';
...
begin
  ...
  try
    loQry:= TFDQuery.Create(self);
    loQry.Connection := conexao;
    loqry.sql.Add('select P.*, S.PESSOA_ID, S.DESCRICAO_SERVICO, S.CONTATO, S.CELULAR AS TEL_CELULAR, S.ATIVO AS PRESTADOR_ATIVO from PESSOAS P, PRESTADORES_SERVICOS S where P.ID = S.PESSOA_ID and P.ID = :ID');
    ...
    edt_telefones.Text          := loQry.FieldByName('TEL_CELULAR').AsString;
    edt_servicos.Text           := loQry.FieldByName('DESCRICAO_SERVICO').AsString;
    cb_fornecedor_ativo.Checked := loQry.FieldByName('PRESTADOR_ATIVO').AsString = 'S';
  finally
    FreeAndNil(loQry)
  end;
end;
  ...
  begin
    try
      try
        ...
        loQuery.SQL.Add('select PESSOA_ID from PRESTADORES_SERVICOS where PESSOA_ID =:PESSOA_ID');
