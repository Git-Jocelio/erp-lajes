...
type
  ...
  public
    ...
    property p_taxa_id :integer read Fp_taxa_id write Fp_taxa_id;
...
begin
  ...
  // -1 para indica que a maquina é da empresa, se maior que 0 pertence a terceiro(algum fornecedor)
  p_proprietario_id := -1;
...
begin
  ...
  {id do proprietario da maquininha igual a -1(maquina da empresa) ,
  se maior que 0 a maquininha é de algum fornecedor}
  if p_proprietario_id = -1 then
  if strtofloatdef(edt_taxa.Text,0) <= 0 then
  begin
    CriarMensagem('AVISO','CARTÃO SELECIONADO NÃO TEM TAXA CADASTRADA.' + sLineBreak + slinebreak +
                  'CADASTRE A TAXA E TENTE NOVAMENTE. ' );
    exit;
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      p_taxa_id  := loQry.FieldByName('id').AsInteger;
