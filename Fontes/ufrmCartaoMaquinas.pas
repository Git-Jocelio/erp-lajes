...
type
  TfrmCartaoMaquinas = class(TfrmBaseGrade)
    ...
    qryNR_MAQUINA: TStringField;
    qryVALOR_ALUGUEL: TFMTBCDField;
    qryDIA_VENCIMENTO: TIntegerField;
    qryMAQUINA_EM_POSSE_DE: TStringField;
...
begin
  ...
  qry.SQL.ADD('select                              ' );
  qry.SQL.ADD('  M.ID,                             ' );
  qry.SQL.ADD('  M.CADASTRADO_EM,                  ' );
  qry.SQL.ADD('  M.ALTERADO_EM,                    ' );
  qry.SQL.ADD('  P2.NOME AS NM_OPERADORA,          ' );
  qry.SQL.ADD('  P1.NOME,                          ' );
  qry.SQL.ADD('  M.NR_MAQUINA,                     ' );
  qry.SQL.ADD('  M.VALOR_ALUGUEL,                  ' );
  qry.SQL.ADD('  M.DIA_VENCIMENTO,                 ' );
  qry.SQL.ADD('  M.MAQUINA_EM_POSSE_DE,            ' );
  qry.SQL.ADD('  M.ATIVO                           ' );
  qry.SQL.ADD('from                                ' );
  qry.SQL.ADD(' CARTAO_MAQUINAS M,                 ' );
  qry.SQL.ADD(' CARTAO_OPERADORAS O,               ' );
  qry.SQL.ADD(' PESSOAS P1, PESSOAS P2             ' );
  qry.SQL.ADD('where                               ' );
  qry.SQL.ADD(' M.CARTAO_OPERADORAS_ID = O.PESSOA_ID  and ' );
  qry.SQL.ADD(' M.PROPRIETARIO_ID      = P1.ID and ' );
  qry.SQL.ADD(' O.PESSOA_ID            = P2.ID     ' );
  qry.Open;
  sSql := qry.SQL.Text;
