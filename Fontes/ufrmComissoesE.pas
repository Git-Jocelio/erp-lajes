...
type
  ...
  public
    ...
    property p_obs    : string read Fp_obs write Fp_obs;

  end;

  function fnc_incluir      : string;
  function Alterar(ACodigo  : integer) : string;
  ...
  try
    ...
    try
      ...
      begin
        ...
        ParamByName('PEDIDO_ID').AsInteger  := Codigo;
...
(*
  ...
  begin
    ShowMessage('informe uma data contábil valida');
    dtp_data_contabil.SetFocus;
    exit;
  end;
*)
  p_obs := mm_obs.Lines.Text;
...
begin
  try
    ...
    qryComissao.SQL.Add('select * from COMISSAO where PEDIDO_ID =:PEDIDO_ID');
    qryComissao.ParamByName('PEDIDO_ID').AsInteger := Codigo;
    qryComissao.Open;
