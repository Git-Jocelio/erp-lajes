...
begin
  try
    ...
    loqry.SQL.Clear;
    loQry.SQL.Add('select p.*, e.id as empresa_id, e.nome_fantasia, e.site, e.telefone1, e.telefone2, e.crea, e.frase, e.IMG_LOGO ');
    loQry.SQL.Add('  from ');
    loQry.SQL.Add('pessoas p, empresa e ');
    loQry.SQL.Add('  where ');
    loQry.SQL.Add(' e.pessoa_id = :id  ');
    ...
    edt_imagem.Text        := loQry.FieldByName('IMG_LOGO').AsString;;
    Image1.Picture.LoadFromFile( edt_imagem.Text );
...
begin
  ...
  try
    ...
    if Self.Conexao.InTransaction then Self.Conexao.Commit;
      CriarMensagem('AVISO','EMPRESA salvo com sucesso!');
...
procedure TfrmEmpresaE.SpeedButton1Click(Sender: TObject);
var
 v_endereco: string;
begin
  inherited;
...
begin
  try
    ...
    if operacao = opExcluir then
    begin
      loQry.SQL.Add('update EMPRESA set ATIVO = ' + QuotedStr('N') + ' where PESSOA_ID =:ID');
      loQry.ParamByName('ID').AsInteger := Codigo;
      loQry.ExecSQL;
      exit;
    end;
    ...
    begin
     ...
     // loQry.SQL.Add('  COND  = :COND,                    ');
      ...
      loQry.ParamByName('ID').AsInteger := qry.FieldByName('ID').AsInteger;
    ...
    loQry.ParamByName('IMG_LOGO').AsString      := edt_imagem.Text;
