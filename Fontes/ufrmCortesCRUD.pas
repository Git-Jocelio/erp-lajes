...
type
  TfrmCortesCRUD = class(TForm)
    ...
    procedure edt_tamanhoKeyPress(Sender: TObject; var Key: Char);
...
procedure TfrmCortesCRUD.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) then
  begin
...
begin
  if fnc_validar then
  begin
    tamanho    := strtofloat( edt_tamanho.Text );
...
procedure TfrmCortesCRUD.edt_tamanhoKeyPress(Sender: TObject; var Key: Char);
begin
...
begin
  ...
  if ((StrToFloatDef(edt_tamanho.text,0) < 1) or
      (StrToFloatDef(edt_tamanho.text,0) >12.00)) then
  begin
    ShowMessage('Informe um tamanho de corte válido, entre 1,00 e 12,00 mts');
    edt_tamanho.SetFocus;
    exit;
  end;

  // pega as duas casas dedimais
  //decimal := copy(edt_tamanho.Text, length(edt_tamanho.Text)-1, length(edt_tamanho.Text) );
  decimal := copy(edt_tamanho.Text, length(edt_tamanho.Text), length(edt_tamanho.Text) );
