...
type
  ...
  private
   ...
   // Fp_pedido: string;
    ...
    Fp_qtde_pedidos_liquidado: integer;
    ...
    Fp_entrega_ini: string;
    Fp_entrega_fim: string;
  ...
  public
    ...
    property p_qtde_pedidos_liquidado: integer read Fp_qtde_pedidos_liquidado write Fp_qtde_pedidos_liquidado;
    ...
    property p_entrega_ini: string read Fp_entrega_ini write Fp_entrega_ini;
    property p_entrega_fim: string read Fp_entrega_fim write Fp_entrega_fim;
...
procedure prc_executa( nome_vendedor :string; qtde_pedidos, qtde_pedidos_pagos, qtde_orcamentos,
                       ...
                       qtde_pedidos_liquidado, qtde_pedidos_aguardando,qtde_pedidos_cancelados: integer;
                       valor_tota_pedidos, valor_pago, valor_a_receber, valor_comissoes: double;
                       cliente, vendedor: integer;  dtEmiIni, dtEmiFim, dtContIni,
                       dtContFim, dtEntIni, dtEntFim,  orcamento, Entregue, Pago : string);
var
 sSql: string;
...
begin
  ...
  loForm.p_qtde_pedidos_liquidado  := qtde_pedidos_liquidado;
  ...
  loForm.p_entrega_ini := dtEntIni;
  loForm.p_entrega_fim := dtEntFim;
...
begin
  ...
  lbl_qtde_pedidos_liquidado.Caption  := inttostr(p_qtde_pedidos_liquidado);
  ...
  // entrega
  if p_entrega_ini <> '-1' then
  begin
    qry.SQL.Add(' and p.data_entrega >= :ent1 and p.data_entrega <= :ent2 ');
    qry.Params.ParamByName('ent1').AsDate := strtodate(p_entrega_ini);
    qry.Params.ParamByName('ent2').AsDate := strtodate(p_entrega_fim);
  end;
  ...
  try
    ...
    if p_entrega_ini <> '-1' then
    begin
      loQry.SQL.Add(' and p.data_entrega >= :ent1 and p.data_entrega <= :ent2 ');
      loQry.Params.ParamByName('ent1').AsDate := strtodate(p_entrega_ini);
      loQry.Params.ParamByName('ent2').AsDate := strtodate(p_entrega_fim);
    end;
...
begin
  try
    ...
    loQry.SQL.Clear;
    loQry.SQL.Add('select                                        ');
    loQry.SQL.Add(' I.PRODUTO_ID,  p.nome_fantasia, p.unidade,   ');
    loQry.SQL.Add(' sum(i.qtde) as total                         ');
    loQry.SQL.Add('from                                          ');
    ...
    if p_entrega_ini <> '-1' then
    begin
      loQry.SQL.Add(' and ped.data_entrega >= :ent1 and ped.data_entrega <= :ent2 ');
      loQry.Params.ParamByName('ent1').AsDate := strtodate(p_entrega_ini);
      loQry.Params.ParamByName('ent2').AsDate := strtodate(p_entrega_fim);
    end;
...
procedure TfrmPedidosTotais.sb_produtosClick(Sender: TObject);
begin
  inherited;
