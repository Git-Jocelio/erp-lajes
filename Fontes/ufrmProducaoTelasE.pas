...
type
  TfrmProducaoTelasE = class(TfrmBaseEdicao)
    ...
    _edt_estoque_disponivel: TDBEdit;
    Label4: TLabel;
    Label5: TLabel;
    edt_unidade: TDBEdit;
    ...
    procedure ds_acoDataChange(Sender: TObject; Field: TField);
  private
...
procedure TfrmProducaoTelasE.ds_acoDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  if operacao = opincluir then
     edt_estoque_disponivel.Text := formatfloat ('0.00',qry_aco.FieldByName('estoque_disponivel').AsFloat);
end;
  ...
  try
    ...
    on E : Exception do
       begin
         ...
         if dmconn.FDConnection.InTransaction then
...
//ShowMessage('ENTREI');
  ...
  begin
//ShowMessage('incluir');
    qry.SQL.Add('insert into ' + Tabela );  // PRODUCAO_TELAS
    qry.SQL.Add('(');
  ...
  begin
    ...
    qry.SQL.Add('  ESTOQUE_ACO           = :ESTOQUE_ACO           ');
    qry.SQL.Add('WHERE                                            ');
    qry.SQL.Add('  ID = :ID                                       ');
    //
    qry.ParamByName('ID').AsInteger := Codigo;
  ...
  qry.ParamByName('ESTOQUE_ACO').AsFloat             := StrToFloat(edt_estoque_disponivel.Text);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        loQry.SQL.Add(' ( :PRODUCAO_ID, :PRODUTO_ID, :PESO_TELA, :QTDE_TELAS, :ACO_ID, :PESO_TOTAL ) returning ID') ;
...
////
      ...
      if mtb_producoes.FieldByName('ID').AsInteger > 0 then
        loqry.ExecSQL;
////
      {Atualiza saldos das telas na tabela de produtos}
      ...
      {lança movimento de entrada das telas na tabela de movimentação de estoques}
    ...
    //atualizar saldo do aço utilizado
    {Atualiza saldos do aço utilizado na tabela de produtos}
    unit_movimenta_estoques.prc_atualizar_estoque(
                                 cbx_tipo_aco.KeyValue,
                                 'SAIDA', 'AÇO UTILIZADO NA PRODUCAO',
                                 mtb_producoes.FieldByName('TOTAL_ACO').VALUE );

    {lança movimento de do aço utilizado na tabela de movimentação de estoques}
    unit_movimenta_estoques.prc_incluir_alterar_movimento( OpIncluir,'SAIDA','PRODUCAO_TELAS', codigo, cbx_tipo_aco.KeyValue, 'AÇO UTILIZADO NA PRODUCAO', mtb_producoes.FieldByName('TOTAL_ACO').VALUE );
...
begin
  try
    ...
    edt_estoque_disponivel.Text := formatfloat('0.00',loQry.FieldByName('ESTOQUE_ACO').Asfloat);
    loQry.Close;
    ...
    loQry.SQL.Add('  ID, PRODUCAO_ID, PRODUTO_ID,  ');
    loQry.SQL.Add('  PESO_TELA, QTDE_TELAS, ACO_ID,');
    loQry.SQL.Add('  PESO_TOTAL                    ');
    loQry.SQL.Add('from                            ');
    loQry.SQL.Add('  PRODUCAO_TELAS_ITENS          ');
    loQry.SQL.Add('where                           ');
