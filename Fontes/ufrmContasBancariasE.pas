...
begin
  ...
  qry_banco.SQL.Clear;
  qry_banco.SQL.Add('select               ');
  qry_banco.SQL.Add('  b.id,              ');
  qry_banco.SQL.Add('  b.cod_banco,       ');
  qry_banco.SQL.Add('  p.nome             ');
  qry_banco.SQL.Add('from                 ');
  qry_banco.SQL.Add('  bancos b,          ');
  qry_banco.SQL.Add('  pessoas p          ');
  qry_banco.SQL.Add('where                ');
  qry_banco.SQL.Add('  p.id = b.pessoa_id ');
  qry_banco.SQL.Add('order by             ');
  qry_banco.SQL.Add('  p.nome             ');

  qry_banco.Open;

  dbg_contas.Columns[0].Visible := false;
...
begin
  ...
  if operacao = OpIncluir then
  begin
    qry.SQL.Add('insert into ' + self.Tabela );//CONTAS_BANCARIAS
    qry.SQL.Add('(');
...
function TfrmContasBancariasE.fnc_validar: boolean;
var loQry : TFDQuery;
begin
  result := false;
