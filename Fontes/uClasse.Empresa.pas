...
uses
  ...
  TEmpresa = class
    ...
    public
      ...
      function fnc_consulta( sql, parametro: string ): TFDQuery;
  end;
...
function TEmpresa.fnc_consulta(sql, parametro: string): TFDQuery;
begin
  try
    ...
    qryConsulta.SQL.Add(' from EMPRESA where nome_fantasia like :texto');
    qryConsulta.ParamByName('texto').AsString := '%' + parametro + '%' ;
    qryConsulta.Open;
    *)
    qryConsulta.SQL.Add(sql);
    //ShowMessage(qryConsulta.SQL.TEXT);
    if parametro<>'' then
    begin
       {Neste caso o parametro será sempre pelo id}
       qryConsulta.Params[0].Value := parametro ;
       //ShowMessage('VALOR DO PARAMETRO ' + PARAMETRO);
    end;
    qryConsulta.Open;
...
begin
  try
    try
      ...
      begin
        qryInserir.SQL.Add('insert into EMPRESA ( id, pessoa_id, nome_fantasia, site, crea, ');
      ...
      begin
        qryInserir.SQL.Add('update EMPRESA set ');
        ...
        //***
        qryInserir.SQL.Add('where pessoa_id = :pessoa_id');
...
begin
  ...
  begin
    ...
    Fconexao.ExecSQL('delete from EMPRESA where id =:id_chave', [id_chave])
  end;
