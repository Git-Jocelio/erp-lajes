...
uses
  ...
  Vcl.DBGrids, unit_calcular_custo_de_lajes, uFinanceiro, ufrmComprasParcelaE ;
...
begin
  try
    if frmComprasParcelaE = nil then
      frmComprasParcelaE := TfrmComprasParcelaE.Create(application);


    frmComprasParcelaE.compra_id     := mtb_parcelas.FieldByName('conta_pagar_id').AsInteger;
    frmComprasParcelaE.nr_documento  := mtb_parcelas.FieldByName('nr_documento').AsString;
    frmComprasParcelaE.dt_vencimento := mtb_parcelas.FieldByName('vencimento').AsDateTime;
    frmComprasParcelaE.parcela_id    := 1;//mtb_parcelas.FieldByName('nr_parcela').AsInteger;
    frmComprasParcelaE.vr_documento  := mtb_parcelas.FieldByName('valor_parcela').AsFloat;

    frmComprasParcelaE.ShowModal;

    // alterar a parcela
    prc_gerar_parcelas(
                       opAlterar,
                       mtb_parcelas.FieldByName('conta_pagar_id').AsInteger,
                       mtb_parcelas.FieldByName('nr_documento').AsString,
                       frmComprasParcelaE.dt_vencimento,
                       mtb_parcelas.FieldByName('valor_parcela').AsFloat, 0,0);

  finally
    freeandnil(frmComprasParcelaE);
  end;
...
begin
  ...
  try
    ...
    on E : Exception do
       begin
         CriarMensagem('AVISO','NÃO FOI POSSIVEL SALVAR A COMPRA.' + slinebreak + slinebreak + E.Message);
