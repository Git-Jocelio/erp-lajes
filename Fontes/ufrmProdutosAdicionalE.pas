...
procedure TfrmProdutosAdicionalE.Custos;
var
  valorMetro: double;
begin
  inherited;
  ...
  qry.FieldByName('CUSTO_LIQUIDO').AsFloat :=
  ...
  if qry.FieldByName('PRECO_VENDA').AsFloat < qry.FieldByName('CUSTO_LIQUIDO').AsFloat then
    qry.FieldByName('PRECO_VENDA').AsFloat :=qry.FieldByName('CUSTO_LIQUIDO').AsFloat;
...
procedure TfrmProdutosAdicionalE.edPercentualExit(Sender: TObject);
begin
  inherited;
  custos;
end;
...
begin
  ...
  uTipos.opIncluir: begin
                       pnTitulo.Caption := 'CADASTRO DE FERRO ADICIONAL';
...
begin
  ...
  if qry.FieldByName('CUSTO_LIQUIDO').AsFloat <= 0 then
  Begin
    custos;
    exit;
  End;
