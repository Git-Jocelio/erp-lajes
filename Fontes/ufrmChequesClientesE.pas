...
uses
  ...
  ufrmCorrentistasE, Vcl.Grids, Vcl.DBGrids, udmConn, uFinanceiro;
...
type
  TfrmChequesClientesE = class(TfrmBaseEdicao)
    ...
    Label15: TLabel;
    pnl_id_conta: TPanel;
    dtp_repassado: TDateTimePicker;
    lbl_repassado: TLabel;
    edt_repassado: TEdit;
    cb_devolvido: TCheckBox;
    dtp_devolvido: TDateTimePicker;
    lbl_receber: TLabel;
    cbx_receber: TDBLookupComboBox;
    qry_devedor: TFDQuery;
    ds_devedor: TDataSource;
    ...
    procedure cb_repassadoClick(Sender: TObject);
    procedure cb_devolvidoClick(Sender: TObject);
...
procedure TfrmChequesClientesE.btn_fecharClick(Sender: TObject);
begin
  inherited;
  close;
end;
...
procedure TfrmChequesClientesE.cb_devolvidoClick(Sender: TObject);
begin
  inherited;
  if cb_devolvido.Checked then
  begin
    dtp_devolvido.Visible := true;
    lbl_receber.Visible := true;
    cbx_receber.Visible := true;
  end else
  if cb_devolvido.Checked = false then
  begin
    dtp_devolvido.Visible := false;
    lbl_receber.Visible := false;
    cbx_receber.Visible := false;
  end;

end;

procedure TfrmChequesClientesE.cb_repassadoClick(Sender: TObject);
begin
  inherited;
  if cb_repassado.Checked then
  begin
    dtp_repassado.Visible := true;
    lbl_repassado.Visible := true;
    edt_repassado.Visible := true;
  end else
  if cb_repassado.Checked = false then
  begin
    dtp_repassado.Visible := false;
    lbl_repassado.Visible := false;
    edt_repassado.Visible := false;
    cb_devolvido.Visible  := false;
  end;
...
procedure TfrmChequesClientesE.FormActivate(Sender: TObject);
begin
  inherited;
  if cb_devolvido.Checked then
  begin
...
begin
  ...
  qry_devedor.Connection := conexao;
  qry_devedor.Open;
end;
...
begin
  ...
  if cb_repassado.Checked then
    if edt_repassado.Text = '' then
  begin
    criarmensagem('AVISO','INFORME PARA QUEM REPASSOU O CHEQUE.');
    edt_repassado.setfocus;
    exit;
  end;

  if cb_devolvido.Checked then
    if cbx_receber.Text = '' then
  begin
    criarmensagem('AVISO','INFORME QUEM PAGARÁ ESTE CHEQUE DEVOLVIDO.');
    cbx_receber.setfocus;
    exit;
  end;
...
begin
  ...
  begin
    ...
    qry.SQL.Add('  DATA_REPASSE,             ');
    qry.SQL.Add('  REPASSADO_PARA,           ');
    qry.SQL.Add('  DEVOLVIDO                 ');
    qry.SQL.Add(')                           ');
    qry.SQL.Add('VALUES                      ');
    qry.SQL.Add('(                           ');
    qry.SQL.Add('  :CADASTRADO_EM,           ');
  ...
  begin
    ...
    qry.SQL.Add('  DATA_REPASSE         = :DATA_REPASSE,         ');
    qry.SQL.Add('  REPASSADO_PARA       = :REPASSADO_PARA,       ');
    qry.SQL.Add('  DATA_DEVOLUCAO       = :DATA_DEVOLUCAO,       ');
    qry.SQL.Add('  DEVEDOR_PESSOA_ID    = :DEVEDOR_PESSOA_ID,    ');
    qry.SQL.Add('  DEVOLVIDO            = :DEVOLVIDO             ');
    qry.SQL.Add('WHERE                                           ');
    qry.SQL.Add('  ID = :ID                                      ');
    //
    qry.ParamByName('ID').AsInteger := codigo;
    qry.ParamByName('DATA_DEVOLUCAO').AsDate       := dtp_repassado.DateTime;
    qry.ParamByName('DEVEDOR_PESSOA_ID').AsInteger := ubiblioteca.SeSenao(cbx_receber.text = '', -1, cbx_receber.KeyValue);
  ...
  qry.ParamByName('DATA_REPASSE').AsDate            := dtp_repassado.DateTime;
  qry.ParamByName('REPASSADO_PARA').AsString        := edt_repassado.Text;
  qry.ParamByName('DEVOLVIDO').AsString             := ubiblioteca.sesenao(cb_devolvido.Checked = true , 'S','N');
  ...
  // caso o cheque foi devolvido, enviar ao contas a receber
  if cb_devolvido.Checked then
  begin
    ufinanceiro.AddContasReceber(conexao, -1, date, cbx_receber.KeyValue, 'CHEQUES_CLIENTES', codigo,
                                1,1,StrToFloat(edt_valor.Text), 0,0,0,0,date,-1,0,
                                'CHEQUE DEV :BANCO ' + pnl_nr_banco.Caption + ' AG. ' + pnl_nr_agencia.Caption +
                                ' CC ' + pnl_nr_conta.Caption + ' NR. ' + edt_nr_cheque.Text,
                                '',57);

     CriarMensagem('AVISO','CHEQUE DEVOLVIDO ENVIADO AO CONTAS A RECEBER COM SUCESSO!');
  end;
