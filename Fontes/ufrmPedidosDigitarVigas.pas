...
begin
  ...
  if strtointdef(edt_quantidade.Text,0) <= 0 then
  begin
    ShowMessage('infome uma quantidade válida');
    edt_quantidade.SetFocus;
    exit;
  end;

  if ((strtofloatdef(edt_tamanho.Text,0) < 0.5) or (strtofloatdef(edt_tamanho.Text,0) > 12)) then
  begin
    ShowMessage('informe um tamanho de viga válido');
    edt_tamanho.SetFocus;
    exit;
  end;


  {formata o tamanho com duas casas decimais}
  edt_tamanho.Text := formatfloat('0.00', StrToFloatDef(edt_tamanho.Text,0));
  {pega o ultimo número da viga}
  n := copy(edt_tamanho.Text,Length(edt_tamanho.Text), Length(edt_tamanho.Text));

  if ((strtoint(n) <> 0) and (strtoint(n) <> 5))  then
