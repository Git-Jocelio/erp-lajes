...
begin
   sql  := '';
   if rgSituacaoPagto.ItemIndex = 0 then pago := 'S'
   else
   if rgSituacaoPagto.ItemIndex = 1 then pago := 'N'
   else
   pago := '';
...
begin
  ...
  begin
    ...
    // Encontra o componente TFrxMemoView no relatório
    MemoView := rel_contas_receber.FindObject('label_pessoa') as TFrxMemoView;
    if MemoView <> nil then
    begin
    ...
    // Encontra o componente TFrxMemoView no relatório
    MemoView := rel_contas_receber.FindObject('lbl_descricao_pessoa') as TFrxMemoView;
    if MemoView <> nil then
    begin
...
procedure TfrmContasReceber.rgOrdemClick(Sender: TObject);
begin
  inherited;
  // só habilita pra pedidos
  if rgOrdem.ItemIndex = 0 then
  begin
