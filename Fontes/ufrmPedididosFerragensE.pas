...
begin
  ...
  qryFerros.SQL.Add('  PRECO_CUSTO,    '); //preço fornecedor
  qryFerros.SQL.Add('  CUSTO_LIQUIDO,  '); // preço fornedor mais as percas
  qryFerros.SQL.Add('  PRECO_VENDEDOR, ');
...
begin
  {carrega componentes}
  edCustoLiquido.Text := FormatFloat('0.00', qryFerros.FieldByName('CUSTO_LIQUIDO' ).AsFloat);
  edCustoFornecedor.Text := FormatFloat('0.00', qryFerros.FieldByName('PRECO_VENDEDOR').AsFloat);
...
begin
  ...
  edCustoLiquido.Text := FormatFloat('0.00', custoMetro);
...
begin
  ...
  custoFornecedor := qryFerros.FieldByName('PRECO_CUSTO').AsFloat;
  custoMetro  := StrToFloat(edCustoFornecedor.Text);
...
begin
  ...
  *)
  ...
  if StrToFloatDef(edCustoFornecedor.Text,0) <= 0 then
  begin
    ShowMessage('informe um valor válido');
    edCustoFornecedor.SetFocus;
    exit;
  end;

  if StrToFloatDef(edVendaMetro.Text,0) < StrToFloatDef(edCustoFornecedor.Text,0) then
  begin
    ShowMessage('informe um valor válido');
  ...
  if strtofloat(edCorpo.Text) <  strtofloat(formatfloat('0.00',TamanhoDaViga)) then
    if cbxPosicao.Text = '' then
    begin
      ShowMessage('Informe a posição do reforço na viga');
      cbxPosicao.SetFocus;
      exit;
    end;
