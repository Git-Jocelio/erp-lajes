...
begin
  ...
  //qryPagamentos.SQL.Add('select  B.CADASTRADO_EM AS DATA, B.ID AS RECIBO_ID, B.HISTORICO, I.VALOR_PAGO AS VALOR from  CONTAS_RECEBER_BAIXA B, CONTAS_RECEBER_BAIXA_ITENS I where B.ID = I.CONTAS_RECEBER_BAIXA_ID AND I.CONTAS_RECEBER_ID =:IDCONTA');
  qryPagamentos.SQL.Add('select  CADASTRADO_EM AS DATA, RECIBO_ID, HISTORICO, VALOR from CONTAS_RECEBER_BAIXA  where CONTAS_RECEBER_ID =:IDCONTA');
  //ShowMessage('filtrar conta : ' + inttostr(Self.Codigo) );
...
begin
  try
    ...
    try
      ...
      dtp_data_contabil.Date := loQuery.fieldbyname('DATA_CONTABIL').AsDateTime;
