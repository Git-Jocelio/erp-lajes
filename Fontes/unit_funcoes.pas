...
uses
  ...
  function fnc_completa_zeros( texto : string;  tam_maximo: word): string;
...
uses unit_mensagem, Vcl.Dialogs;//  unit_conexao;
...
begin
  ...
  begin
    ...
    if ( UpperCase( xDBGrid.Fields[I].FieldName ) = 'NOME' ) or
      ...
      // ( UpperCase( xDBGrid.Fields[I].FieldName ) = 'NOME_CLIENTE')or
...
begin
  ...
  for I := 0 to length( trim(texto) ) do
    if (texto[i] in ['0'..'9']) then  str := str + texto[i];

    Result := stringofchar('0', tam_maximo-length(str))  + str;
end;
