...
begin
  ...
  btnImprimir.Enabled := (not qry.IsEmpty) and cb_pessoa.Checked;
end;
...
begin
  ...
  if cb_cadatrado_em.Checked then
    Condicao := Condicao + ' and CP.CADASTRADO_EM >= :CAD_INI and CP.CADASTRADO_EM <= :CAD_FIM ';
  ...
  begin
    ...
    qry.SQL.Text := qry.SQL.Text + situacao + Condicao ;
    ...
    {Carregar parametros}
    if cb_pessoa.Checked then
      qry.ParamByName('NOME').AsString := cbx_pessoas.Text;
     ...
     if cb_cadatrado_em.Checked then
      begin
        qry.ParamByName('CAD_INI').AsDate := dtp_emissao_ini.Date;
        qry.ParamByName('CAD_FIM').AsDate := dtp_emissao_fim.Date;
      end;
  ...
  begin
    ...
    qry.SQL.Text := qry.SQL.Text + situacao ;
