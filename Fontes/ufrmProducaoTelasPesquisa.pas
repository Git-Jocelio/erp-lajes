...
type
  ...
  public
    ...
    property tela_peso : double read Ftela_peso write Ftela_peso;
...
begin
  ...
  begin
    ...
    tela_peso          := qry.FieldByName('PESO').AsFloat;
...
begin
  ...
  begin
    criarmensagem('ERRO', 'SELECIONE A TIPO DE TELA PRODUZIDA.');
  ...
  begin
    criarmensagem('ERRO', 'INFORME UMA QUANTIDADE DE TELA(S) VÁLIDA.');
...
begin
  ...
  qry.Connection := conexao;
  qry.SQL.Clear;
  qry.SQL.Add('select id, nome_fantasia, unidade, peso from produtos where nome_fantasia like :nome_fantasia order by nome_fantasia');
  qry.ParamByName('nome_fantasia').AsString := '%' + 'TELA' + '%';
  qry.Open;
