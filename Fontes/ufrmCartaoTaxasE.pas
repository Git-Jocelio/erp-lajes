...
begin
  ...
  try
    ...
    frmCartaoTaxasItens.up_vencimento.Associate  := frmCartaoTaxasItens.edt_Vencimento;
    frmCartaoTaxasItens.edt_Vencimento.Text      := mt_itensPRIMEIRO_VENCIMENTO.AsString;

    frmCartaoTaxasItens.up_parcela.Associate     := frmCartaoTaxasItens.edt_intervalo;
...
begin
  ...
  if operacao = OpIncluir then
  begin
    qry.SQL.Add('insert into ' + self.Tabela );// CARTAO_TAXAS
    qry.SQL.Add('(');
...
function TfrmCartaoTaxasE.fnc_validar: boolean;
var
  loQry: TFDQuery;
begin
  result := false;
